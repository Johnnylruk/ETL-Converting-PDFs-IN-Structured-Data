...
import pandas as pd
import logging
from Files.config.tools.postgre import RDSPostgreSQLManager

from unidecode import unidecode
from Files.config.rules.rules import rules_dict

logging.basicConfig(level = logging.INFO)

class PDFTableExtractor:
	def __init__(self, file_name, configs):
		self.path = os.path.abspath(f"D:\Data Engineer Journey\ETL-ConverterPDF\Files\{configs["name"].lower()}\{file_name}.pdf")
		self.csv_path = os.path.abspath(f"D:\Data Engineer Journey\ETL-ConverterPDF\Files\csv")
...
# Fixing Table headers 
	...
	def send_to_db(df, table_name):
		try:
			...
			logging.info(f"Data saved on Database {table_name}")
...
def list_files(folder):
	try:
		...
		logging.info(f"Folder {folder} cannot be found.")
	except Exception as e:
...
if __name__ == "__main__":
	...
	corretora = 'redex'
	path = os.path.abspath(f"D:\Data Engineer Journey\ETL-ConverterPDF\Files\{corretora}\\")
